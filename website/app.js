...
generateKey.addEventListener("click", async () => {
  ...
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?zip=${zipCode}&appid=${theWeatheAppKey}&units=metric`
  )
    .then((response) => {
    ...
    }).then((finalFormOfData)=>{
      displayResults(finalFormOfData)
    });
}); //
...
async function displayResults(finalFormOfData) {
  ...
  try {
    ...
    document.getElementById("temp").innerHTML = "Today temprature is " + theData.temp.toFixed(1) +
    " °C which equal to " + ((theData.temp * 9) / 5 + 32).toFixed(1) + " °F and The humidity is " + theData.humidity +"% .";
    document.getElementById("content").innerHTML = "I heared that you are " + theData.content + " . So did I.";
    document.getElementById("country").innerHTML = "This zip code you enter is in " + theData.place + " , " + theData.country;
    // if error happan.
  } catch (error) {
